...
knitr::opts_chunk$set(echo = FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 9, fig.align = "center")
```
...
## A new suite of senior mathematics and science subjects {#new-subjects}
...
```{r mean-enrolments-all, fig.cap = "Average enrolments for all schools for each year, enrolments for the new QCE system are coloured in a darker shade of grey"}
# Compute total Year 12 enrolments for each year, by subject
mean_enrolments_all <- mathsci_all %>% 
  group_by(completion_year, subject_name, subject) %>% 
  summarise(mean_year12_enrolments = mean(year_12_enrolments),
            .groups = "drop") 
...
mean_enrolments_all %>% 
  ...
  geom_col(aes(x = completion_year,
               y = mean_year12_enrolments),
           fill = "grey80") +
  # Enrolments for from 2019 (new system)
  geom_col(aes(x = completion_year,
               y = mean_year12_enrolments),
           fill = "grey30",
           data = filter(mean_enrolments_all, completion_year > 2019)) + 
  facet_wrap(~ subject_name,
             scales = "free_y") +
  ...
  labs(title = "Average (mean) year 12 enrolments for each subject per cohort",
       x = "Completion Year",
...
```{r time-plot-all, fig.cap = "Time plot of enrolments of all the subjects across the years"}
mathsci_all_long %>% 
...
```{r density-plots, fig.cap="Distributions of enrolments for each subject shown with a density plot, overlaid with a density plot"}
library(ggridges)
...
```{r boxplot-dist, fig.cap="Box-plot of distribution of enrolments for each subject by sector"}
...
$$\begin{aligned} Y_{ij} & = \pi_{0ij} + \epsilon_{tij} \\
         & = (\beta_{00j} + u_{0ij}) + \epsilon_{tij} \\
         & = (\gamma_{000}) + (r_{00j} + u_{0ij} + \epsilon_{tij}) \end{aligned}$$
...
$$\begin{aligned} \epsilon_{tij} & \sim N(0, \sigma^2) \\ u_{0ij} & \sim N(0, \tau^2_{00}) \\ 
r_{00j} & \sim N(0, \phi^2_{00}) \end{aligned}$$
...
$$\begin{aligned} Y_{tij} &= \pi_{0ij} + \pi_{1ij}year_{tij} + \epsilon_{tij} \\ &= (\beta_{00j} + u_{0ij}) + (beta_{10j} + u_{1ij})year_{tij} +  \epsilon_{tij} \\
&= (\gamma_{000} + r_{00j} + u_{0ij})  + (\gamma_{100} + r_{10j} + u_{1ij})year_{tij} + \epsilon_{tij} \\
&= [\gamma_{000} + \gamma_{100}year_{tij}] + [r_{00j} + u_{0ij} + \epsilon_{tij} + (r_{10j} + u_{1ij})year_{tij}] \end{aligned}$$
...
As the report is constrained to a maximum of 10,000 words, only the enrolments for the four mathematics subjects will be considered. Please refer to [this GitHub](https://github.com/BrendiA/ETC5543-proj) for the predictions for of science subjects.

```{r, child="models/mathematical-methods.Rmd", eval=TRUE}
```

```{r, child="models/specialist-mathematics.Rmd", eval=TRUE}
```

```{r, child="models/essential-mathematics.Rmd", eval=TRUE}
```

```{r, child="models/general-mathematics.Rmd", eval=TRUE}
```

# Predictions for Science Subjects

As the report is constrained to a maximum of 10,000 words, only the enrolments for the four mathematics subjects will be considered. Please refer to [this GitHub](https://github.com/BrendiA/ETC5543-proj) for the predictions for the predictions of science subjects.

```{r, child="models/agricultural-practices.Rmd", eval=FALSE}
```

```{r, child="models/agricultural-science.Rmd", eval=FALSE}
```

```{r, child="models/aquatic-practices.Rmd", eval=FALSE}
```

```{r, child="models/biology.Rmd", eval=FALSE}
```

```{r, child="models/chemistry.Rmd", eval=FALSE}
```

```{r, child="models/earth-and-environmental-science.Rmd", eval=FALSE}
```

```{r, child="models/marine-science.Rmd", eval=FALSE}
```

```{r, child="models/physics.Rmd", eval=FALSE}
```

```{r, child="models/science-in-practice.Rmd", eval=FALSE}
```
