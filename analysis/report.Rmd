...
```{r nested-structure, fig.cap = "Nested structure of the dataset", out.width = "80%", out.height = "130%"}
...
- Level two (schools within clusters)
$$\begin{aligned} \pi_{0ij} &= \beta_{00j} + u_{0ij} \\ \pi_{1ij} &= \beta_{10j} + u_{1ij} \end{aligned}$$
  
- Level three (clusters)
$$\begin{aligned}\beta_{00j} &= \gamma_{000} + r_{00j} \\ \beta_{10j} &= \gamma_{100} + r_{10j} \end{aligned}$$
...
```{r, child="models/agricultural-practices.Rmd", eval=TRUE}
```

```{r, child="models/agricultural-science.Rmd", eval=TRUE}
```

```{r, child="models/aquatic-practices.Rmd", eval=TRUE}
```

```{r, child="models/biology.Rmd", eval=TRUE}
```

```{r, child="models/chemistry.Rmd", eval=TRUE}
```

```{r, child="models/earth-and-environmental-science.Rmd", eval=TRUE}
```

```{r, child="models/marine-science.Rmd", eval=TRUE}
```

```{r, child="models/physics.Rmd", eval=FALSE}
```

```{r, child="models/science-in-practice.Rmd", eval=TRUE}
```
