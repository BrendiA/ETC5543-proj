...
mathsci_all_long %>% 
  filter(enrolments > 0) %>% 
  ggplot() +
  geom_boxplot(aes(x = subject_name, 
                   y = enrolments,
                   fill = unit)) +
...
corrected_enrol %>% 
  ggplot(aes(y = adj_intcp,
             x = adj_slope)) +
  geom_point() +
  labs(x = "Adjusted slope (change in enrolments per unit increase in completion_year)",
...
spec_math_long <- spec_math_long %>% 
  mutate(c_completion_year = completion_year - mean(completion_year))
```
...
postcode_lmer <- lme4::lmer(enrolments ~ c_completion_year + unit + (c_completion_year | school_postcode), 
           data = spec_math_long)
``` 
