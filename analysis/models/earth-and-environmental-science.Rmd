...
```{r}
# Time plot of 20 randomly selected schools
set.seed(403)
earth_sci %>% 
...
- Some schools only had few students enrolling in the subject --- *e.g.* school 128, 198 etc.
...
$$ICC(school) = \frac{\tau^{2}_{00}}{\tau^{2}_{00} + \phi^{2}_{00} + \sigma^2} = \frac{1.5173}{(1.5173 + 0.2951)} = 0.8372$$
...
model_anova %>% 
  ...
  arrange(AIC)
```
- Based on the AIC, `model4.4` is the best.
...
save(bootstrapLRT, file = here::here("data/bootstrapLRT/earth-sci-model5.0-vs-model5.1.rda"))
```

```{r}
load(here::here("data/bootstrapLRT/earth-sci-model5.0-vs-model5.1.rda"))
...
- There is overwhelming statistical evidence ($\chi^2$ = 143.537 and $p$-value = 0)
...
```{r}
log_enrolments ~ year92 + sector + year92:sector + unit + (year92 | qcaa_school_id)
```
...
Y_{tij} =\ & \pi_{0ij} + \pi_{1ij}year92_{tij} + \epsilon_{tij} \\ 
        ...
        =\ & \left[\beta_{00j} + \beta_{01j}sector_{ij} + \beta_{02}unit_{ij} + \beta_{10j}year92_{tij} + \beta_{11j}sector_{ij}year92_{tij} \right] + 
             \left[u_{0ij} + u_{1ij} + \epsilon_{tij} \right]
