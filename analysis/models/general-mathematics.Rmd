...
- Based on the AIC values, the the model where schools are nested within districts is the best
...
- Naturally, the higher-order random effects (*i.e.* random slope of the third level (between district)) can be removed
...
- As shown in the model output, there was negligible change in the fixed effects/


## Parametric bootstrap (correlation for largel model producing NaN (check again))

```{r}
# ANOVA is wrong
anova(model1.1, model1.0)
```
...
anova(model1.0_ml, model1.1_ml)

2*(logLik(model1.0_ml)- logLik(model1.1_ml))
```
...
save(bootstrapLRT, file = here::here("data/bootstrapLRT/gen-math-model1.0-vs-model1.1.rda"))
```

```{r}
# Significant at the 5% level, but not at the 1% level
# Can see if this still holds when we add predictors
as_tibble(bootstrapLRT)
```

The parametric bootstrap ($\chi^2$-statistic = 4.3119 and $p$-value = 0.023) is significant at the 5% level, but not significant at the 1% level. Therefore, at the 5% level, the larger model (with random slope for district) is favoured.

```{r}
load(file = here::here("data/bootstrapLRT/gen-math-model1.0-vs-model1.1.rda"))
...
ggplot(nullLRT.1) + 
  ...
  geom_vline(xintercept = 4.311875, 
             size = 1,
             colour = "red") + 
...
- Based on AIC, the model favours the largest model `model4.0`
...
model_f <- lmer(log_enrolments ~ year92*sector*unit +
              ...
              data = gen_math_long)
# Expands to 
model_f <- lmer(log_enrolments ~ year92 + sector + unit + year92:sector + year92:unit + sector:unit + year92:sector:unit +
       (year92 | qcaa_district:qcaa_school_id) + 
       (1 | qcaa_district),
       REML = TRUE,
       control = lmerControl(optimizer = "bobyqa"),
