...
- Based on the AIC values, the model with schoools nested within districts will be used.
...
$$ICC(school) = \frac{\tau^{2}_{00}}{\tau^{2}_{00} + \phi^{2}_{00} + \sigma^2} = \frac{0.5294}{(0.5294 + 0.1178 + 0.2188)} = 0.6113$$

The **level-three ICC** refers to the correlation between different schools $i$ and $i^*$ from a specific district $j$. 

$$ICC(school) = \frac{\phi^2_{00}}{\tau^2_{00} + \phi^2_{00} + \sigma^2} = \frac{0.1178}{(0.5294 + 0.1178 + 0.2188)} = 0.1360$$
...
- Seems like the best model is the one without the interaction `year92:unit`
- Based on the AIC, the best model is `model4.7`
...
save(bootstrapLRT, file = here::here("data/bootstrapLRT/phys-model5.0-vs-model5.1.rda"))
```

```{r}
load(here::here("data/bootstrapLRT/phys-model5.0-vs-model5.1.rda"))
```
...
- The bootstrap likelihood ratio test is significant at the 1% level ($\chi^2$ = 7.2276, $p$-value = 0.006), suggesting that the bigger model is the better alternative.
