...
*to edit: reference to table*
The parametric bootstrap is utilised to construct confidence intervals (as detailed in [step 8](#step-eight)). If the confidence intervals for the random effects does not include 0, it provides statistical evidence that the p-value is less than 0.5. In other words, it suggests that the random effects and the correlation between the random effects are significant at the 5% level.  The 95% confidence interval is shown above (Table \@ref(tab:bio-confint)), and the random effects all exclude 0, further reiterating that they are statistically significant at the 5% level.
...
```{r phys-fixef, fig.cap = "Fixed effects of the final model for Biology"}
fit_f %>% 
  ggplot(aes(x = completion_year,
             y = fit,
             group = sector_unit,
