...
- There are 94 schools nested in 69 postcodes
- And 94 schools nested within 13 districts
- Based on the AIC, the model with schools nested within postcodes is the best.
...
The **level-two ICC** is the correlation between a school $i$ from a certain postcode $j$ in time $t$ and in time $t^*$:

$$ICC(school) = \frac{\tau^{2}_{00}}{\tau^{2}_{00} + \phi^{2}_{00} + \sigma^2} = \frac{0.4052}{(0.4052 + 0.1228 + 0.2918)} = 0.4943$$

The **level-three ICC** refers to the correlation between different schools $i$ and $i^*$ from a specific postcode $j$. 

$$ICC(school) = \frac{\phi^2_{00}}{\tau^2_{00} + \phi^2_{00} + \sigma^2} = \frac{0.1228}{(0.4052 + 0.1228 + 0.2918)} = 0.1498$$
...
```{r}
model2.0 <- lmer(log_enrolments ~ year94 + sector + sector*year94 +
                (year94 | school_postcode:qcaa_school_id) + 
                (year94 | school_postcode),
                REML = TRUE,
                control = lmerControl(optimizer = "bobyqa"),
...
```{r}
model3.0 <- lmer(log_enrolments ~ year94 + sector + sector*year94 + unit + unit:year94 +
                (year94 | school_postcode:qcaa_school_id) + 
                (year94 | school_postcode),
                REML = TRUE,
                control = lmerControl(optimizer = "bobyqa"),
...
save(bootstrapLRT, file = here::here("data/bootstrapLRT/agri-sci-model5.0-vs-model5.1.rda"))
```

```{r}
load(here::here("data/bootstrapLRT/agri-sci-model5.0-vs-model5.1.rda"))
...
```{r}
log_enrolments ~ year94 + sector + unit + year94:unit + (year94 | school_postcode:qcaa_school_id) + (year94 | school_postcode)
```
...
# Government (unit 11)
fit_gov_unit11 <-
  fixef_f[[1]] + fixef_f[[3]] + fixef_f[[2]] * c(0:30) + fixef_f[[6]] * c(0:30)
...
# Independent (unit 11)
fit_ind_unit11 <-
  fixef_f[[1]] + fixef_f[[4]] + fixef_f[[2]] * c(0:30) + fixef_f[[6]] * c(0:30)
