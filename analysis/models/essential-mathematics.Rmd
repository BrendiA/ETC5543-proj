...
ess_math_long <- ess_math_long %>%
  # Remove 22 observations with 0 enrolments
  filter(enrolments != 0) %>% 
  # Recentre year
...
- According to the AIC, model with schools nested within districts is the best
...
$$ICC(school) = \frac{\tau^{2}_{00}}{\tau^{2}_{00} + \phi^{2}_{00} + \sigma^2} = \frac{1.1484}{(1.1484 + 0.0477 + 0.1140)} = 0.8766$$

The **level-three ICC** refers to the correlation between different schools $i$ and $i^*$ from a specific district $j$. 

$$ICC(school) = \frac{\phi^2_{00}}{\tau^2_{00} + \phi^2_{00} + \sigma^2} = \frac{0.0477}{(1.1484 + 0.0477 + 0.1140)} = 0.0364$$
...
```{r}
model2.0 <- lmer(log_enrolments ~ year20 + sector + sector*year20 +
                (year20 | qcaa_district:qcaa_school_id) + 
                (year20 | qcaa_district),
                REML = TRUE,
                data = ess_math_long)
...
model_anova %>% 
  ...
  arrange(AIC)
```
- Based on the AIC, the largest possible model (`model4.0`) is the best
...
# Null model
bnull <- lmer(log_enrolments ~ year20 + sector + year20:sector + sector:unit + unit + 
               (year20 | qcaa_district:qcaa_school_id) +      
               (1 | qcaa_district),
              REML = FALSE,
              data = ess_math_long)
...
save(bootstrapLRT, file = here::here("data/bootstrapLRT/ess-maths-model5.0-vs-model5.1.rda"))
```

```{r}
# Load bootstrap likelihood ratio test output
load(here::here("data/bootstrapLRT/ess-maths-model5.0-vs-model5.1.rda"))
...
- Overwhelming statistical evidence supporting the larger model.
...
- Level three (districts)
  $$\beta_{00j} = \gamma_{000} + r_{00j} \\
    ...
    \beta_{12j} = \gamma_{120} + r_{12j} \\ 
    \beta_{13j} = \gamma_{130} + r_{13j}$$
...
Y_{tij} =\ &\pi_{0ij} + \pi_{1ij}year92_{tij} + \epsilon_{tij} \\
        ...
        =\ & \left[\gamma_{000} + r_{00j} + (\gamma_{010} + r_{01j})sector_{ij} + (\gamma_{020} + r_{02j})unit_{ij} + 
            ...
            & \left[\gamma_{100} + r_{10j} + (\gamma_{110} + r_{11j})sector_{ij} + (\gamma_{120} + r_{12j})unit_{ij} + 
              (\gamma_{130} + r_{13j})sector_{ij}unit_{ij} + u_{1ij} \right]year20_{tij} \\
              
        =\ & \left[\gamma_{000} + \gamma_{010}sector_{ij} + \gamma_{020}unit_{ij} \gamma_{030}sector_{ij}unit_{ij} +
            \gamma_{100}year20_{tij} + \gamma_{110}sector_{ij}year_{tij} \gamma_{120}unit_{ij}year20_{ij} + 
             \gamma_{130}sector_{ij}unit_{ij}year20_{tij} \right] \\
           & \left[r_{00j} + r_{01j}sector_{ij} + r_{02j}unit_{ij} + r_{03j}sector_{ij}unit_{ij} + r_{10j}year20_{tij} + 
            r_{11j}sector_{ij}year20_{tij} + r_{12j}unit_{ij}year20_{tij} + r_{13j}sector_{ij}unit_{ij}year20_{tij} +
             u_{0ij} + u_{1ij}year20_{tij}\right] 
...
# \gamma_{100}year20_{tij} + \gamma_{110}sector_{ij}year_{tij} \gamma_{120}unit_{ij}year20_{ij} + 
# \gamma_{130}sector_{ij}unit_{ij}year20_{tij}
...
# Government (unit 12)
fit_gov_unit12 <-
  fixef_f[[1]] + fixef_f[[3]] + fixef_f[[5]] + fixef_f[[9]] + fixef_f[[2]] * c(0:5) + 
  fixef_f[[6]] * c(0:5) + fixef_f[[8]] * c(0:5) + fixef_f[[11]] * c(0:5)
...
# Independent (unit 12)
fit_ind_unit12 <-
  fixef_f[[1]] + fixef_f[[4]] + fixef_f[[5]] + fixef_f[[10]] + fixef_f[[2]] * c(0:5) +
  fixef_f[[7]] * c(0:5) + fixef_f[[8]] * c(0:5) + fixef_f[[12]] * c(0:5)
```
